/* ============================================================
   ...
   NEW: Payment gateway toggle (Manual UPI / Cashfree Payments),
        UPI ID management, live preview, payments module upgrade,
        UTR verification workflow. Cashfree v3 hosted checkout integration.
   ============================================================ */
...
const openDrawer = content => {
  ...
  document.body.classList.add('drawer-open'); // v13 — CSS-only scroll lock
...
// v13 SCROLL SAFETY NET — same watchdog as the storefront: if the
// drawer close path ever errors before restoring body.style.overflow,
// this reopens page scroll automatically within one second.
(() => {
  try {
    // Clear any lingering scroll-lock left by a previous crashed session
    document.body.style.overflow = '';
    document.body.classList.remove('folium-no-scroll','drawer-open','modal-open');
    document.documentElement.style.overflow = '';
    document.documentElement.style.overflowY = 'auto';

    const scrollWatchdog = () => {
      const b = document.body;
      const drawerOpen = !!document.querySelector('#drawer:not([hidden]), #scrim:not([hidden])');
      if (!drawerOpen) {
        if (b.style.overflow === 'hidden') b.style.overflow = '';
        b.classList.remove('drawer-open','folium-no-scroll','modal-open');
      }
    };
...
const loadSettings = async () => {
  ...
  // Cashfree credentials live in an ADMIN-ONLY doc (settingsPrivate/payment)
  // so the publicly-readable settings/site doc can never leak the secret key.
  // Merge them into S.settings.payment so the settings form fills as before.
  try {
    const priv = await db.collection('settingsPrivate').doc('payment').get();
    const pub = S.settings.payment || {};
    const p = priv.exists ? (priv.data() || {}) : {};
    S.settings.payment = {
      ...pub,
      cashfreeAppId: p.cashfreeAppId || pub.cashfreeAppId || '',
      cashfreeSecretKey: p.cashfreeSecretKey || pub.cashfreeSecretKey || '', // legacy fallback (older saves kept it in the public doc)
      cashfreeEnv: p.cashfreeEnv || pub.cashfreeEnv || ((p.cashfreeLiveMode || pub.cashfreeLiveMode) ? 'production' : 'sandbox'),
      cashfreeLiveMode: (p.cashfreeLiveMode != null ? p.cashfreeLiveMode : pub.cashfreeLiveMode) || (p.cashfreeEnv === 'production' || pub.cashfreeEnv === 'production'),
      cashfreeTheme: p.cashfreeTheme || pub.cashfreeTheme || '#2F5CFF'
    };
  } catch (e) { console.warn('Private payment settings unavailable (rules not deployed yet?):', e); }
};
...
const Orders = (() => {
  ...
  const methodLabel = m => m === 'manual_upi' ? '📱 UPI' : m === 'cashfree' ? '⚡ Cashfree' : (m === 'razorpay' ? '⚡ Razorpay (legacy)' : '—');
  ...
  const render = () => {
    ...
    const filtered = orders.filter(o => {
      ...
      if (f.q) {
        const hay = `${o.id} ${o.userEmail || ''} ${o.customer?.email || ''} ${o.templateName || ''} ${o.template?.name || ''} ${o.utr || ''} ${o.cashfreePaymentId || o.cashfreeOrderId || o.razorpayPaymentId || ''}`.toLowerCase();
  ...
  const showDetail = async id => {
    try {
      ...
      [
        ...
        ...kv('UTR / Reference', o.utr || o.cashfreePaymentId || o.razorpayPaymentId || '—'),
        ...kv('UPI VPA used', o.upiVpaUsed || '—'),
        ...kv('Cashfree order id', o.cashfreeOrderId || '—'),
        ...kv('Cashfree payment id', o.cashfreePaymentId || '—'),
        ...kv('Cashfree payment session', o.cashfreePaymentSessionId || '—'),
  ...
  const exportCsv = () => {
    ...
    const header = ['order_id','customer','template','amount','currency','method','status','utr','cashfree_order_id','cashfree_payment_id','created_at','updated_at'];
    ...
    rows.forEach(o => lines.push([
      ...
      o.utr || '',
      o.cashfreeOrderId || '',
      o.cashfreePaymentId || o.razorpayPaymentId || '',
...
const Payments = (() => {
  ...
  const methodLabel = m => m === 'manual_upi' ? '📱 Manual UPI' : m === 'cashfree' ? '⚡ Cashfree' : (m === 'razorpay' ? '⚡ Razorpay (legacy)' : '—');
  ...
  const render = () => {
    ...
    rows.forEach(o => {
      const tr = el('tr', {},
        el('td', {}, o.cashfreePaymentId || o.razorpayPaymentId || o.utr || '—'),
        el('td', {}, `#${o.id.slice(0, 8)}`),
        el('td', {}, methodLabel(o.paymentMethod)),
        el('td', {}, o.utr || o.cashfreePaymentId || o.razorpayPaymentId || '—'),
        el('td', {}, o.amount != null ? `${fmtINR(o.amount)}` : '—'),
  ...
  const showProof = async id => {
    ...
    [
      ...
      ['UTR', o.utr || '—'],
      ['Cashfree order id', o.cashfreeOrderId || '—'],
      ['Cashfree payment id', o.cashfreePaymentId || o.razorpayPaymentId || '—'],
  ...
  const exportCsv = () => {
    ...
    rows.forEach(o => {
      lines.push([
        o.id,
        o.paymentMethod || '',
        o.utr || o.cashfreePaymentId || o.razorpayPaymentId || '',
        o.amount || 0,
        o.currency || 'INR',
        o.status || '',
...
const Settings = (() => {
  const load = async () => {
    ...
    if (payForm) {
      ...
      // Cashfree fields
      if (payForm.elements.cashfreeAppId) payForm.elements.cashfreeAppId.value = pay.cashfreeAppId || '';
      if (payForm.elements.cashfreeSecretKey) payForm.elements.cashfreeSecretKey.value = pay.cashfreeSecretKey || '';
      if (payForm.elements.cashfreeEnv) payForm.elements.cashfreeEnv.value = pay.cashfreeEnv || (pay.cashfreeLiveMode ? 'production' : 'sandbox');
      if (payForm.elements.cashfreeLiveMode) payForm.elements.cashfreeLiveMode.checked = !!pay.cashfreeLiveMode;
      if (payForm.elements.cashfreeTheme) payForm.elements.cashfreeTheme.value = pay.cashfreeTheme || '#2F5CFF';
  ...
  const applyPaymentModeUI = mode => {
    ...
    card.classList.toggle('mode-cashfree', mode === 'cashfree');
    const manual = $('#pg-block-manual');
    const cf = $('#pg-block-cashfree');
    if (manual) manual.classList.toggle('active', mode === 'manual_upi');
    if (cf) cf.classList.toggle('active', mode === 'cashfree');
  ...
  // v6: HARDENED — if the QR CDN fails, lazy-inject it; if the canvas render
  // still fails, fall back to a public QR image so the admin ALWAYS sees the
  // same preview the customer will see on the storefront.
  const ensureQrLib = () => new Promise((resolve) => {
    ...
    s.async = true; s.setAttribute('data-qr-lib', '1');
  ...
  const refreshUpiPreview = () => {
    ...
    const fallbackImg = () => {
      // Replace canvas with an <img> fallback in-place so the layout doesn't jump.
      let img = document.getElementById('upi-preview-qr-img');
      if (!img) {
        img = document.createElement('img');
        img.id = 'upi-preview-qr-img';
        img.width = 132; img.height = 132;
        img.alt = 'UPI QR preview (fallback)';
        img.style.borderRadius = '8px'; img.style.background = '#fff';
        canvas.insertAdjacentElement('afterend', img);
      }
      img.src = `https://api.qrserver.com/v1/create-qr-code/?size=132x132&margin=1&data=${encodeURIComponent(upiUrl)}`;
      img.style.display = '';
      canvas.style.display = 'none';
      setQrDiag('✓ QR live (fallback renderer — customers see this exact code).', false);
    };
    const render = () => {
      // Hide any previous fallback <img> so we don't stack two QRs.
      const img = document.getElementById('upi-preview-qr-img'); if (img) img.style.display = 'none';
      try {
        window.QRCode.toCanvas(canvas, upiUrl, { width: 132, margin: 1, color: { dark: '#0A0E1A', light: '#FFFFFF' } }, err => {
          if (err) { fallbackImg(); return; }
          canvas.style.display = '';
          setQrDiag('✓ QR live — this is exactly what customers scan.', false);
        });
      } catch (e) { fallbackImg(); }
    };
    if (window.QRCode && typeof window.QRCode.toCanvas === 'function') render();
    else ensureQrLib().then(ok => { if (ok) render(); else fallbackImg(); });
  };
  ...
  if (payForm) {
    ...
    payForm.addEventListener('submit', async e => {
      ...
      try {
        ...
        const cashfreeAppId = String(fd.get('cashfreeAppId') || '').trim();
        const cashfreeSecretKey = String(fd.get('cashfreeSecretKey') || '').trim();
        const cashfreeEnv = String(fd.get('cashfreeEnv') || 'sandbox').trim();
        ...
        if (mode === 'cashfree') {
          if (!cashfreeAppId || cashfreeAppId.length < 8) throw new Error('Enter a valid Cashfree App ID (from Merchant Dashboard → Developers → API Keys).');
          if (!cashfreeSecretKey || cashfreeSecretKey.length < 10) throw new Error('Enter a valid Cashfree Secret Key (starts with cfsk_ma_test_ or cfsk_ma_prod_).');
          if (!['sandbox', 'production'].includes(cashfreeEnv)) throw new Error('Choose a valid Cashfree environment.');
        }

        // liveMode auto-syncs with env choice
        const cashfreeLiveMode = (cashfreeEnv === 'production');

        // PUBLIC doc — readable by the storefront. NO credentials here.
        const paymentPublic = {
          mode,
          upiId,
          upiPayeeName,
          ...
          // Non-secret Cashfree display config (env + theme only)
          cashfreeEnv,
          cashfreeLiveMode,
          cashfreeTheme: fd.get('cashfreeTheme') || '#2F5CFF',
          updatedAt: firebase.firestore.FieldValue.serverTimestamp(),
          updatedBy: S.user?.uid || null
        };
        // ADMIN-ONLY doc — the Cloud Function reads App ID / Secret from here.
        ...
        await save({ payment: paymentPublic });
        await db.collection('settingsPrivate').doc('payment').set(paymentPrivate, { merge: true });
        // Scrub any legacy credentials that older versions wrote into the public doc.
        try {
          await db.collection('settings').doc('site').update({
            'payment.cashfreeSecretKey': firebase.firestore.FieldValue.delete(),
            'payment.cashfreeAppId': firebase.firestore.FieldValue.delete()
          });
        } catch (e) { /* doc may not exist yet — fine */ }
        // Keep local state in sync so re-opening the form shows saved values.
        S.settings = S.settings || {};
        S.settings.payment = { ...paymentPublic, cashfreeAppId, cashfreeSecretKey };
        if (status) { status.className = 'form-note ok'; status.textContent = `✅ Payment mode set to ${mode === 'manual_upi' ? 'Manual UPI' : 'Cashfree (' + cashfreeEnv + ')'}. Live on storefront.`; }
