/* ============================================================
   DUDE — Customer site (FULLY FIXED v5 — Cashfree Payments)
   Changes vs v4:
   - Payment gateway migrated from Razorpay to Cashfree Payments v3
   - createOrder / verifyPayment Cloud Functions now return
     { cashfree_order_id, payment_session_id, order_amount, order_currency }
   - Uses window.Cashfree(...).checkout({ paymentSessionId }) SDK v3 flow
   - Order docs write cashfreeOrderId / cashfreePaymentSessionId / cashfreePaymentId
   - Everything else (Manual UPI, templates, orders, reviews) unchanged.
   ============================================================ */
...
// Wait until Firebase Auth has restored the session and a fresh ID token is
// available — callable functions reject with UNAUTHENTICATED if the cached
// token is stale.
const waitForFreshAuth = () => new Promise((resolve, reject) => {
  if (auth.currentUser) {
    auth.currentUser.getIdToken(true).then(() => resolve(auth.currentUser)).catch(reject);
    return;
  }
  const off = auth.onAuthStateChanged(async user => {
    try { off(); } catch {}
    if (!user) return reject(new Error('Please sign in to pay.'));
    try { await user.getIdToken(true); resolve(user); } catch (e) { reject(e); }
  });
  setTimeout(() => { try { off(); } catch {}; reject(new Error('Sign-in session timed out. Please re-login.')); }, 12000);
});

// Cashfree JS SDK v3 loader — the storefront HTML often forgets the
// <script src="https://sdk.cashfree.com/js/v3/cashfree.js"> tag, which leaves
// window.Cashfree undefined. Load it on demand, idempotently.
const ensureCashfreeSDK = () => new Promise((resolve, reject) => {
  if (typeof window.Cashfree === 'function') return resolve(true);
  const existing = document.querySelector('script[data-cashfree-sdk]');
  if (existing) {
    existing.addEventListener('load', () => resolve(typeof window.Cashfree === 'function'), { once: true });
    existing.addEventListener('error', () => reject(new Error('Could not load the Cashfree SDK. Check your connection/ad-blocker.')), { once: true });
    return;
  }
  ...
  s.onload = () => (typeof window.Cashfree === 'function')
    ? resolve(true)
    : reject(new Error('Cashfree SDK loaded but did not initialise.'));
  s.onerror = () => reject(new Error('Could not load the Cashfree SDK. Check your connection/ad-blocker.'));
...
const Data = (() => {
  ...
  // FIX (v6): Firestore rules require `list` queries to include the SAME
  // where-clause the rule enforces (`isActive == true`), otherwise every anon
  // visitor is blocked with `permission-denied` — which is why logged-in
  // ADMINS were seeing plans but public visitors were seeing nothing.
  // We now query `where('isActive','==',true)` (indexed automatically as a
  // single-field query, no composite index needed) and fall back to an
  // unfiltered read if the collection is fully public.
  const loadCategories = async () => {
    const finalize = docs => {
      State.categories = docs
        .map(d => ({ id: d.id, ...(d.data ? d.data() : d) }))
    ...
    try {
      ...
      return finalize(snap.docs);
    } catch (e) {
      console.warn('Filtered categories failed, retrying open:', e && e.code);
      try {
  ...
  // FIX (v6): Firestore rules require the list query itself to include the
  // enforced `isActive == true` clause — otherwise anonymous visitors get
  // `permission-denied` and the storefront looks empty for everyone who
  // isn't a logged-in admin. We add it to every path (with a graceful
  // fallback for open collections).
  const loadTemplatesPage = async (categoryId, sort) => {
    const finalize = docs => docs
      .map(d => ({ id: d.id, ...(d.data ? d.data() : d) }))
      .filter(t => t.isActive !== false);
    const runQuery = async (withActive) => {
      let q = db.collection('templates');
      if (categoryId) q = q.where('categoryId', '==', categoryId);
      if (withActive) q = q.where('isActive', '==', true);
    ...
    try {
      ...
      try { docs = await runQuery(true); }
        ...
        // Composite index missing OR rules mismatch — try again without the
        // active filter (works when the collection is fully public or admin).
        console.warn('Filtered template list failed, retrying open:', e1 && e1.code);
        docs = await runQuery(false);
      }
      let items = finalize(docs);
  ...
  // FIX (v6): Same story — customForms list rule requires `enabled == true`
  // in the query itself for anonymous / logged-in customers to list them.
  // Without this the checkout was silently rendering with zero fields for
  // some categories on public devices.
  ...
  const loadFormFields = async categoryId => {
    ...
    try {
      const snap = await db.collection('customForms').where('enabled', '==', true).get();
      return finalize(snap.docs);
    } catch (e) {
      console.warn('Filtered customForms failed, retrying open:', e && e.code);
      try {
        const snap = await db.collection('customForms').get();
        return finalize(snap.docs);
      } catch (err) {
        console.error('Form fields load failed:', err);
        return [];
      }
    }
  };
  ...
  // ---- LIVE SYNC (v5) -----------------------------------------------------
  // The storefront used to read settings & categories ONCE at boot, so admin
  // edits (prices, plans, payment config, brand) never appeared until a full
  // refresh. These realtime listeners push every admin change instantly.
  const listenSettings = cb => {
    try {
      return db.collection('settings').doc('site').onSnapshot(snap => {
        State.settings = snap.exists ? { ...DEFAULT_SETTINGS, ...snap.data() } : { ...DEFAULT_SETTINGS };
        if (typeof cb === 'function') cb(State.settings);
      }, err => console.warn('Settings listener error:', err));
    } catch (e) { console.warn('listenSettings failed:', e); return () => {}; }
  };

  const listenCategories = cb => {
    const apply = docs => {
      State.categories = docs
        .map(d => ({ id: d.id, ...(d.data ? d.data() : d) }))
        .filter(c => c.isActive !== false)
        .map(c => ({ ...c, order: (typeof c.order === 'number' ? c.order : 9999) }))
        .sort((a, b) => (a.order - b.order) || String(a.name || '').localeCompare(String(b.name || '')));
      if (typeof cb === 'function') cb(State.categories);
    };
    const attach = (withActive) => {
      let q = db.collection('categories');
      if (withActive) q = q.where('isActive', '==', true);
      return q.onSnapshot(
        snap => apply(snap.docs),
        err => {
          console.warn('Categories listener error:', err && err.code);
          // Rules disallowed the un-filtered listen — retry with the filter.
          if (withActive === false && err && /permission/i.test(err.message || err.code || '')) {
            try { attach(true); } catch {}
          }
        }
      );
    };
    try { return attach(true); }
    catch (e) { console.warn('listenCategories failed:', e); return () => {}; }
  };
  ...
  return {
    ...
    listenSettings, listenCategories
  };
})();
...
const Auth = (() => {
  ...
  const openModal = () => {
    ...
    document.body.classList.add('modal-open'); // v13 scroll lock
  };
  const closeAllModals = () => {
    ...
    document.body.classList.remove('modal-open'); // v13
  ...
  const wire = () => {
    ...
    // FIX (Google login): complete the sign-in when the browser RETURNS from
    // the Google account chooser via the redirect flow. Without handling
    // getRedirectResult(), users pick their Google account, come back to the
    // site, and are NOT logged in. Also pin persistence so the session sticks.
    (async () => {
      try {
        try { await auth.setPersistence(firebase.auth.Auth.Persistence.LOCAL); } catch (e) { console.warn('setPersistence failed:', e); }
        const result = await auth.getRedirectResult();
        if (result && result.user) {
          await Data.ensureUserDoc(result.user);
          closeAllModals();
          Util.toast('Signed in with Google.', 'success');
          let target = null;
          try { target = sessionStorage.getItem('dude_post_login_route'); sessionStorage.removeItem('dude_post_login_route'); } catch {}
          if (target) setTimeout(() => { location.hash = target; }, 100);
        }
      } catch (err) {
    ...
    Util.$('#google-signin')?.addEventListener('click', async () => {
      const status = Util.$('#auth-status');
      if (status) { status.className = 'form-note'; status.textContent = 'Opening Google sign-in…'; }
      const provider = new firebase.auth.GoogleAuthProvider();
      provider.setCustomParameters({ prompt: 'select_account' });
      const fallbackToRedirect = async () => {
        try {
          // Persist intended route across the full-page redirect
          if (State.postLoginRoute) {
            try { sessionStorage.setItem('dude_post_login_route', State.postLoginRoute); } catch {}
          }
          await auth.signInWithRedirect(provider); // page navigates away; result handled above on return
        } catch (err) {
          console.error('Google redirect fallback failed:', err);
          if (status) { status.className = 'form-note err'; status.textContent = 'Google sign-in failed. Please try again.'; }
          Util.toast(String(err && err.message || 'Sign-in failed').replace('Firebase: ', ''), 'error');
        }
      };
      try {
        const cred = await auth.signInWithPopup(provider);
        if (!cred || !cred.user) { await fallbackToRedirect(); return; }
        await Data.ensureUserDoc(cred.user);
        if (status) status.textContent = '';
        closeAllModals();
        Util.toast('Signed in with Google.', 'success');
        ...
        const code = String(err && err.code || '');
        // User closed the popup themselves — stay silent, keep modal open
        if (/popup-closed-by-user|cancelled-popup-request/.test(code)) {
          if (status) status.textContent = '';
          return;
        }
        // Domain not whitelisted in Firebase console — redirect would fail the
        // same way, so surface a clear configuration error instead
        if (/unauthorized-domain/.test(code)) {
          if (status) { status.className = 'form-note err'; status.textContent = 'This domain is not authorized for Google sign-in. Add it in Firebase Console → Authentication → Authorized domains.'; }
          Util.toast('Domain not authorized for Google sign-in (see Firebase authorized domains).', 'error');
          return;
        }
        // Popup blocked / impossible (mobile browsers, COOP headers, in-app
        // webviews, storage partitioning) → fall back to full-page redirect
        if (/popup-blocked|operation-not-supported-in-this-environment|web-storage-unsupported|internal-error/.test(code) || /popup/i.test(code)) {
...
const Templates = (() => {
  ...
  const cardFor = t => {
    ...
    const _imgUrl = u => (String(u || '').startsWith('data:image/') ? u : Util.safeUrl(u));
    const thumb = _imgUrl(t.thumbnailUrl) || _imgUrl(t.images && t.images[0]);
    if (thumb) {
  ...
  // FIX (v6): Public visitors browse freely — the sign-in gate ONLY fires
  // when they actually attempt to buy. We open the auth modal FIRST (feels
  // instant), then queue the intended checkout so it resumes after login.
  // Auth boot may not have finished on very slow devices; if State.user is
  // pending, wait one auth tick before deciding.
  const handleBuy = (id) => {
    const proceed = () => {
      if (!State.user) {
        State.postLoginRoute = `#/checkout/${id}`;
        Auth.openModal();
        // Toast AFTER the modal is on-screen so it doesn't shift focus.
        setTimeout(() => Util.toast('Please sign in to buy this template.', 'info'), 40);
        return;
      }
      location.hash = `#/checkout/${id}`;
    };
    if (!State.bootDone) {
      const off = auth.onAuthStateChanged(() => { try { off(); } catch {} proceed(); });
      // Safety timeout so the button always responds.
      setTimeout(() => { try { off(); } catch {} proceed(); }, 1200);
      return;
    }
    proceed();
  };
...
const Detail = (() => {
  ...
  const setGallery = t => {
    // Support both hosted URLs and embedded Base64 (data:) images from the
    // admin Base64 fallback path.
    const keepUrl = u => (String(u || '').startsWith('data:image/') ? u : Util.safeUrl(u));
    const raw = (t.images && t.images.length) ? t.images : (t.thumbnailUrl ? [t.thumbnailUrl] : []);
    const images = raw.map(keepUrl).filter(Boolean);
    ...
    primary.onclick = () => {
      ...
      scrim.hidden = false;
      document.body.style.overflow = 'hidden';
      document.body.classList.add('modal-open'); // v13
    };
  ...
  // Renders the template video: uploaded mp4/webm (Storage / data:) plays
  // inline; YouTube & Vimeo links become responsive embeds.
  const setVideo = t => {
    const wrap = Util.$('#detail-video-wrap');
    const frame = Util.$('#detail-video-frame');
    if (!wrap || !frame) return;
    frame.innerHTML = '';
    const raw = String(t.videoUrl || '').trim();
    if (!raw) { wrap.hidden = true; return; }

    const ytMatch = raw.match(/(?:youtube\.com\/(?:watch\?v=|shorts\/|embed\/)|youtu\.be\/)([A-Za-z0-9_-]{6,})/);
    const vimeoMatch = raw.match(/vimeo\.com\/(\d+)/);

    if (ytMatch) {
      frame.appendChild(Util.el('iframe', {
        src: `https://www.youtube-nocookie.com/embed/${ytMatch[1]}`,
        allow: 'accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture',
        allowfullscreen: true, loading: 'lazy', title: `${t.name || 'Template'} video`
      }));
      wrap.hidden = false;
      return;
    }
    ...
    // Direct video file (Storage URL, data:, or any .mp4/.webm link)
    const src = raw.startsWith('data:video/') ? raw : Util.safeUrl(raw);
    if (src) {
      frame.appendChild(Util.el('video', { src, controls: true, playsinline: true, preload: 'metadata' }));
      wrap.hidden = false;
    } else {
      wrap.hidden = true;
    }
  ...
  const updateSEO = t => {
    ...
    const title = `${t.name || 'Template'} — ${brand} Portfolio Templates`;
    const desc = (t.shortDescription || `Buy ${t.name || 'this'} portfolio template — responsive, source code included, instant delivery.`).slice(0, 160);
    document.title = title;
    const setMeta = (sel, val) => { const n = document.querySelector(sel); if (n) n.setAttribute('content', val); };
    const setHref = (sel, val) => { const n = document.querySelector(sel); if (n) n.setAttribute('href', val); };
    setMeta('meta[name=description]', desc);
    setMeta('#og-title', title);
    setMeta('#og-desc', desc);
    setMeta('#tw-title', title);
    setMeta('#tw-desc', desc);
    const img = Util.safeUrl(t.thumbnailUrl) || Util.safeUrl(t.images && t.images[0]) || '';
    if (img) { setMeta('#og-image', img); setMeta('#tw-image', img); }
    setHref('#canonical-link', location.href.split('#')[0] + location.hash);
    setMeta('#og-url', location.href);

    // Product schema — rich results in Google (price, ratings, availability)
    try {
      const price = (t.discountPrice && t.discountPrice < t.price) ? t.discountPrice : (t.price || 0);
      const product = {
        '@context': 'https://schema.org',
        '@type': 'Product',
        name: t.name || 'Portfolio Template',
        description: desc,
        ...
        offers: {
          ...
          priceCurrency: 'INR',
          price: String(price),
          availability: t.isActive === false ? 'https://schema.org/OutOfStock' : 'https://schema.org/InStock',
          url: location.href
        }
      };
      if (t.rating && t.rating.count > 0) {
        product.aggregateRating = {
          ...
          ratingValue: String((t.rating.average || 0).toFixed(1)),
          reviewCount: String(t.rating.count)
        };
      }
    ...
    } catch (e) { /* schema best-effort */ }
  };
  const resetSEO = () => {
    const brand = State.settings?.brandName || 'DUDE';
    document.title = `${brand} — Ready-Made Portfolio Websites for Students & Developers`;
    // Remove product-specific schema when leaving the detail view.
  ...
  const render = async id => {
    ...
    try {
      ...
      // FIX (v6): Public visitor sees the Buy button + a subtle hint that
      // sign-in happens at checkout, NOT before. This matches the intent:
      // browse freely, gate at purchase.
      const buyBtn = Util.$('#buy-now');
      if (buyBtn) {
        buyBtn.onclick = () => Templates.handleBuy(t.id);
        const parent = buyBtn.parentElement;
        if (parent) {
          const existing = parent.querySelector('.buy-signin-hint');
          if (existing) existing.remove();
          if (!State.user) {
            const hint = Util.el('small', { class: 'buy-signin-hint' }, '🔒 You\'ll be asked to sign in at checkout — takes 20 seconds.');
            buyBtn.insertAdjacentElement('afterend', hint);
          }
        }
      }
...
const Checkout = (() => {
  ...
  const submit = async () => {
    ...
    try {
      ...
      // ==== Cashfree Payments flow (v3) ====
      if (!functions) throw new Error('Online payments are not configured yet.');
      let res;
      try {
        await waitForFreshAuth();
        res = await functions.httpsCallable('createOrder')({ orderId: orderRef.id, templateId: t.id, gateway: 'cashfree' });
      } catch (fnErr) {
        console.error('createOrder failed:', fnErr);
        // Surface the real server message (Cashfree misconfig, template price
        // missing, etc.) instead of the generic 'internal'.
        throw new Error(fnErr && fnErr.message && fnErr.message !== 'internal'
          ? fnErr.message
          : 'Online payment could not be initialised. Please try again in a moment.');
      }
      const data = res.data || {};
      const paymentSessionId = data.payment_session_id || data.paymentSessionId;
      const cfOrderId = data.cashfree_order_id || data.order_id;
      const cfAmount = data.order_amount || data.amount;
      const cfCurrency = data.order_currency || data.currency || 'INR';
      const cfMode = (data.mode || payCfg.cashfreeEnv || (payCfg.cashfreeLiveMode ? 'production' : 'sandbox'));
      if (!paymentSessionId) throw new Error('Payment could not be initialized (no payment session).');
      pendingOrderId = null;

      // Persist Cashfree references on the order for admin visibility
      try {
        await orderRef.update({
          paymentMethod: 'cashfree',
          cashfreeOrderId: cfOrderId || null,
          cashfreePaymentSessionId: paymentSessionId,
          amount: cfAmount || null,
          currency: cfCurrency,
          status: 'payment_pending',
          updatedAt: firebase.firestore.FieldValue.serverTimestamp()
        });
      } catch (e) { console.warn('Order pre-payment update failed:', e); }

      try { await ensureCashfreeSDK(); } catch (sdkErr) { throw sdkErr; }

      const cashfree = window.Cashfree({ mode: cfMode === 'production' ? 'production' : 'sandbox' });
      try {
        ...
        if (result && result.paymentDetails) {
          // Verify server-side using Cashfree's Get Order API
          try {
            await waitForFreshAuth();
          ...
          // User closed / dismissed modal without paying
          Util.toast('Payment window closed.', 'info');
        }
      } catch (err) {
        console.error('Cashfree checkout threw:', err);
        Util.toast('Payment failed. No money captured.', 'error');
      } finally {
        restorePayBtn();
      }
...
const Orders = (() => {
  ...
  const paint = async orders => {
    ...
    for (const o of orders) {
      ...
      // ---- Delivery & live status (published by admin) ----
      if (o.liveStatus || o.deliveryUrl || o.deliveryNote || (o.deliveryFile && o.deliveryFile.url)) {
        const del = Util.el('div', { class: 'order-delivery' });
        if (o.liveStatus) {
          const ls = String(o.liveStatus);
          const map = { building: ['🔨', 'Building'], deployed: ['📦', 'Deployed'], live: ['🟢', 'Live'], maintenance: ['🛠', 'Maintenance'] };
          const [ico, label] = map[ls] || ['📡', ls];
          const row = Util.el('div', { class: 'delivery-live-row' });
          row.appendChild(Util.el('span', { class: `live-pill live-${ls}` }, `${ico} ${label}`));
          if (o.deliveryUrl && Util.safeUrl(o.deliveryUrl)) {
            row.appendChild(Util.el('a', { class: 'btn btn-ghost delivery-open', href: Util.safeUrl(o.deliveryUrl), target: '_blank', rel: 'noopener' }, '🔗 Open live site'));
          }
          del.appendChild(row);
        } else if (o.deliveryUrl && Util.safeUrl(o.deliveryUrl)) {
          del.appendChild(Util.el('a', { class: 'btn btn-ghost delivery-open', href: Util.safeUrl(o.deliveryUrl), target: '_blank', rel: 'noopener' }, '🔗 Open live site'));
        }
        if (o.deliveryNote) del.appendChild(Util.el('p', { class: 'delivery-note' }, String(o.deliveryNote)));
        if (o.deliveryFile && o.deliveryFile.url) {
          const size = o.deliveryFile.size ? ` · ${(o.deliveryFile.size / (1024 * 1024)).toFixed(1)} MB` : '';
          del.appendChild(Util.el('a', {
            class: 'btn btn-primary delivery-download',
            href: o.deliveryFile.url, target: '_blank', rel: 'noopener',
            download: o.deliveryFile.name || 'deliverable'
          }, `⬇ Download source code${size}`));
        }
...
const Payment = (() => {
  ...
  const setBlock = id => {
    ['pg-block-manual', 'pg-block-cashfree', 'pg-block-done'].forEach(b => {
      const n = Util.$('#' + b);
  ...
  const bindManual = (o, t) => {
    ...
    // ---- Per-app UPI deep links (v6) --------------------------------------
    // Each chip opens ITS OWN app with the payment pre-filled. The generic
    // upi:// link opens the phone's native app chooser for "More apps".
    const appLink = (scheme) => {
      if (!upiUrl) return '';
      const params = upiUrl.slice(upiUrl.indexOf('?') + 1); // pa, pn, am, cu, tn
      ...
      return upiUrl; // 'more' & fallback → system chooser shows ALL UPI apps
    };
    const fireApp = (scheme, chipEl) => {
      const href = appLink(scheme);
      if (!href || chipEl.classList.contains('is-disabled')) {
        Util.toast('UPI ID not configured yet.', 'info');
        return;
      }
      try { window.location.href = href; } catch (e) {}
      // Fallback: if the specific app isn't installed, the scheme silently
      // fails — after a beat, offer the generic chooser instead.
      if (scheme !== 'more') {
        setTimeout(() => {
          if (!document.hidden) {
            Util.toast('App not found? Use "More apps" to pick any UPI app.', 'info');
          }
        }, 1600);
      }
    };
    ...
    Util.$$('.upi-app-chip').forEach(chip => {
      const scheme = chip.dataset.upiApp || 'more';
      if (upiUrl) {
        chip.onclick = () => fireApp(scheme, chip);
    ...
    // ---- Auto-generated QR (v6) ------------------------------------------
    // The QR is ALWAYS generated locally from the UPI ID — no admin upload
    // needed. HARDENED for the case where the QR library CDN is blocked
    // (school Wi-Fi, ad-blockers, spotty mobile networks): we lazy-inject the
    // script when missing, then retry, and if the canvas render still fails
    // we fall back to a public QR image endpoint so customers ALWAYS see a
    // scannable code.
    const qrWrap = Util.$('#upi-qr-wrap');
    const qrImg = Util.$('#upi-qr-img');
    const qrCanvas = Util.$('#upi-qr-canvas');

    const ensureQrLib = () => new Promise((resolve) => {
      if (window.QRCode && typeof window.QRCode.toCanvas === 'function') return resolve(true);
      // Idempotent: only inject once even if payment view re-renders.
      const existing = document.querySelector('script[data-qr-lib]');
      if (existing) {
        existing.addEventListener('load', () => resolve(!!(window.QRCode && window.QRCode.toCanvas)), { once: true });
      ...
      s.onload = () => resolve(!!(window.QRCode && window.QRCode.toCanvas));
      s.onerror = () => resolve(false);
      document.head.appendChild(s);
    });

    const paintFallbackImg = () => {
      if (!qrWrap || !qrImg || !upiUrl) return false;
      // Public QR image endpoint (no key required, HTTPS, no cookies).
      const src = `https://api.qrserver.com/v1/create-qr-code/?size=240x240&margin=8&data=${encodeURIComponent(upiUrl)}`;
      qrImg.src = src;
      qrImg.alt = 'Scan this QR with any UPI app to pay';
      qrImg.hidden = false;
    ...
    if (qrWrap) {
      if (!upiUrl) {
        ...
        qrWrap.hidden = false;
        const qrSize = Math.max(180, Math.min(240, Math.floor(window.innerWidth * 0.55)));
        const renderCanvas = () => {
          if (!qrCanvas) return paintFallbackImg();
          try {
            window.QRCode.toCanvas(qrCanvas, upiUrl, { width: qrSize, margin: 2, color: { dark: '#0A0E1A', light: '#FFFFFF' } }, err => {
              if (err) { paintFallbackImg(); return; }
              qrCanvas.hidden = false;
              if (qrImg) qrImg.hidden = true;
            });
          } catch (e) { paintFallbackImg(); }
        };
        if (window.QRCode && typeof window.QRCode.toCanvas === 'function') {
          renderCanvas();
        } else {
          ensureQrLib().then(ok => { if (ok) renderCanvas(); else paintFallbackImg(); });
        }
      }
    }
  ...
  const startCashfree = async o => {
    const btn = Util.$('#cf-pay-btn');
    const status = Util.$('#cf-status');
    ...
    try {
      ...
      await waitForFreshAuth();
      const res = await functions.httpsCallable('createOrder')({ orderId: o.id, templateId: o.templateId, gateway: 'cashfree' });
      const data = res.data || {};
      const paymentSessionId = data.payment_session_id || data.paymentSessionId;
      const cfOrderId = data.cashfree_order_id || data.order_id;
      const cfAmount = data.order_amount || data.amount;
      const cfCurrency = data.order_currency || data.currency || 'INR';
      const p = pay();
      const cfMode = data.mode || p.cashfreeEnv || (p.cashfreeLiveMode ? 'production' : 'sandbox');
      if (!paymentSessionId) throw new Error('Payment could not be initialized.');
      await ensureCashfreeSDK();

      // Persist references so admin sees this attempt
      try {
        await db.collection('orders').doc(o.id).update({
          paymentMethod: 'cashfree',
          cashfreeOrderId: cfOrderId || null,
          cashfreePaymentSessionId: paymentSessionId,
          amount: cfAmount || o.amount || null,
          currency: cfCurrency,
          updatedAt: firebase.firestore.FieldValue.serverTimestamp()
        });
      } catch (e) { console.warn('Order update pre-cashfree failed:', e); }
      ...
      if (result && result.error) {
        done('Payment failed. ' + (result.error.message || 'No money captured.'), false);
        return;
      }
      if (result && result.paymentDetails) {
        try {
        ...
        done('Payment window closed.', false);
      }
    } catch (err) {
      console.error('Cashfree start failed:', err);
  ...
  const render = async orderId => {
    ...
    try {
      ...
      const m = mode();
      if (sub) sub.textContent = m === 'cashfree' ? 'Pay securely via Cashfree Payments.' : 'Pay via UPI and submit your UTR.';
      if (m === 'cashfree') {
        setBlock('pg-block-cashfree');
        const btn = Util.$('#cf-pay-btn');
        if (btn) btn.onclick = () => startCashfree(o);
      } else {
  ...
  const wireMobileDelegation = () => {
    ...
    const fireChipTap = (chip) => {
      const generic = (Util.$('#upi-pay-app') && Util.$('#upi-pay-app').getAttribute('href')) || '';
      if (!generic || generic === '#' || chip.classList.contains('is-disabled')) {
        Util.toast('UPI ID not configured yet.', 'info');
        return;
      }
      // Same per-app mapping as the desktop handler — read the params from the
      // generic link and prefix the right scheme for the tapped app.
      const scheme = chip.dataset.upiApp || 'more';
      const params = generic.slice(generic.indexOf('?') + 1);
      const href = scheme === 'gpay' ? `tez://upi/pay?${params}`
        : scheme === 'phonepe' ? `phonepe://pay?${params}`
        : scheme === 'paytm' ? `paytmmp://pay?${params}`
        : generic; // 'more' → system chooser lists every UPI app on the phone
      try { window.location.href = href; } catch (e) {}
    };
...
const UI = (() => {
  ...
  // v6: World-class SEO — canonical & OG url track the active route so
  // Google/Bing index deep links (templates, pricing, reviews) as separate
  // pages, and social shares resolve to the actual view instead of the
  // homepage. We rewrite the tags whenever the hash route changes.
  const updateRouteSEO = (name, param) => {
    try {
      const origin = location.origin && location.origin !== 'null' ? location.origin : '';
      const path = origin + '/' + (location.hash || '#/');
      const setContent = (sel, val) => { const n = Util.$(sel); if (n) n.setAttribute('content', val); };
      const setHref = (sel, val) => { const n = Util.$(sel); if (n) n.setAttribute('href', val); };
      setHref('#canonical-link', path);
      setContent('#og-url', path);
      const routeTitles = {
        home:      { t: 'DUDE — Ready-Made Portfolio Websites', d: 'Handcrafted, developer-grade portfolio templates. Buy, customize, deploy in a weekend.' },
        templates: { t: 'All Portfolio Templates — DUDE', d: 'Browse every ready-made portfolio template — student, developer, designer & freelancer.' },
        pricing:   { t: 'Pricing — DUDE Portfolio Templates', d: 'Simple, transparent pricing tiers. Source code included on every template.' },
        reviews:   { t: 'Verified Reviews — DUDE', d: 'What students, freelancers and indie devs say about DUDE portfolio templates.' },
        contact:   { t: 'Contact DUDE Support', d: 'Questions before you buy? We reply within one business day on WhatsApp.' },
        orders:    { t: 'Your Orders — DUDE', d: 'Track your portfolio template orders and downloads.' }
      };
      const cur = routeTitles[name];
      if (cur) {
        document.title = cur.t;
        const dm = document.querySelector('meta[name="description"]');
        if (dm) dm.setAttribute('content', cur.d);
        setContent('#og-title', cur.t);
        setContent('#og-desc', cur.d);
        setContent('#tw-title', cur.t);
        setContent('#tw-desc', cur.d);
      }
      // BreadcrumbList JSON-LD — live-updated
      const crumb = { '@context': 'https://schema.org', '@type': 'BreadcrumbList', itemListElement: [{ '@type': 'ListItem', position: 1, name: 'Home', item: '/' }] };
      if (name && name !== 'home') {
        crumb.itemListElement.push({ '@type': 'ListItem', position: 2, name: name.charAt(0).toUpperCase() + name.slice(1), item: path });
      }
      const ld = Util.$('#ld-breadcrumb');
      if (ld) ld.textContent = JSON.stringify(crumb);
  ...
  const wireDrawer = () => {
    ...
    const openDrawer = () => {
      ...
      document.body.style.overflow = 'hidden';
      document.body.classList.add('drawer-open'); // v13
    };
    const closeDrawer = () => {
      ...
      document.body.style.overflow = '';
      document.body.classList.remove('drawer-open'); // v13
      if (lastFocus && document.contains(lastFocus)) { try { lastFocus.focus(); } catch {} }
      lastFocus = null;
    };
  ...
  const wireReveal = () => {
    ...
    scan();
    // v6: throttle the mutation observer — the original re-ran `scan()` on
    // every DOM mutation (dozens per template render), which pinned CPU on
    // low-end phones and caused visible scroll jank. Batch into one rAF.
    let scheduled = false;
...
(async function boot() {
  try {
    ...
    document.body.style.overflow = '';
    // v13 scroll fix — clear any stray scroll-lock classes left by an
    // interrupted modal/drawer/chatbot close from a previous session.
    document.body.classList.remove('folium-no-scroll','modal-open','drawer-open','dude-bot-fullscreen-open');
    // Guarantee scroll is enabled at boot even if something goes wrong.
    document.documentElement.style.overflow = '';
    document.documentElement.style.overflowY = 'auto';
  } catch (_) {}

  // v13 — SCROLL SAFETY NET: watchdog that reopens page scroll if it
  // ever gets stuck because a modal/drawer close path threw an error
  // before restoring body.style.overflow. Runs at 1 Hz — cheap.
  try {
    const scrollWatchdog = () => {
      const b = document.body;
      const anyModalOpen = !!document.querySelector(
        '#modal-scrim:not([hidden]), #drawer-scrim:not([hidden]), #mobile-drawer:not([hidden]), .modal:not([hidden])'
      );
      const chatFullscreen = b.classList.contains('dude-bot-fullscreen-open') &&
        !document.querySelector('#dude-bot-panel:not([hidden])');
      if (!anyModalOpen && b.style.overflow === 'hidden') {
        b.style.overflow = '';
      }
      if (chatFullscreen) {
        b.classList.remove('dude-bot-fullscreen-open');
      }
    };
    setInterval(scrollWatchdog, 1000);
    // Also run on route change and page-show
    window.addEventListener('hashchange', scrollWatchdog);
    window.addEventListener('pageshow', scrollWatchdog);
  } catch (_) {}
  ...
  try {
    ...
    // ---- LIVE SYNC (v5): push admin edits to the storefront in realtime ----
    // Settings (brand, contact, payment config) — re-skin + re-render contact
    Data.listenSettings(() => {
      try {
        UI.applyBrand();
        Home.renderContact();
      } catch (e) { console.warn('Live settings apply failed:', e); }
    });
    // Categories / plans — re-render hero, tabs and pricing cards instantly
    Data.listenCategories(() => {
      try {
        const active = State.activeCategoryId ? State.categories.find(c => c.id === State.activeCategoryId) : null;
        // If the active category was removed/deactivated by admin, fall back.
        if (State.activeCategoryId && !active) State.activeCategoryId = State.categories[0]?.id || null;
        const cat = State.activeCategoryId ? State.categories.find(c => c.id === State.activeCategoryId) : null;
        Home.applyHero(cat);
        Home.renderTabs();
        Home.renderPricing();
        // Refresh the grid too — category tabs may have changed the filter set
        try { Templates.reset(); Templates.load(); } catch (e) {}
      } catch (e) { console.warn('Live categories apply failed:', e); }
    });

    // When the shopper returns to the tab, pull fresh template data so admin
    // edits (price, images, videos) appear without a manual reload.
    document.addEventListener('visibilitychange', () => {
      if (document.visibilityState === 'visible' && State.bootDone) {
        try { Templates.reset(); Templates.load(); } catch (e) {}
      }
    });
  } catch (err) {
...
(() => {
  ...
  const KB = [
    ...
    { keys: ['payment','pay','upi','razorpay','cashfree'], reply: () => `We accept UPI (GPay/PhonePe/Paytm) and Cashfree Payments for cards, netbanking &amp; wallets.` },
