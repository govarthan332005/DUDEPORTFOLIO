...
const Detail = (() => {
  ...
  const updateSEO = t => {
    ...
    // og:image must be an absolute hosted URL — Base64 data: URLs are skipped.
...
const Payment = (() => {
  ...
  const bindManual = (o, t) => {
    ...
    // ---- Auto-generated QR (v5) ------------------------------------------
    // The QR is ALWAYS generated locally from the UPI ID — no admin upload
    // needed — and rendered large & centered for easy scanning.
    const qrWrap = Util.$('#upi-qr-wrap');
    const qrImg = Util.$('#upi-qr-img');
    const qrCanvas = Util.$('#upi-qr-canvas');
    if (qrImg) qrImg.hidden = true; // legacy uploaded QR is no longer used
    if (qrWrap) {
      if (upiUrl && window.QRCode && typeof window.QRCode.toCanvas === 'function' && qrCanvas) {
